...
public class Superkoalio implements Screen {
    ...
    private final PrimerSuperKoalio game;
    ...
    Cell celdaMoneda = new Cell();
    int contadorMuerte = 0;
    public static int modo = 1;
    public static int vidas = 3;

    private Label.LabelStyle textStyleV;
    private Label textLblV;
    private Container containerV;

    private Stage stage;

    public Superkoalio(PrimerSuperKoalio game) {
        this.game = game;
    ...
    private void updateKoala (float deltaTime) {
        ...
        TiledMapTileSet tileSet = map.getTileSets().getTileSet("tileSet-hd");
        celdaMoneda.setTile(tileSet.getTile(63));
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                ...
                if (koala.velocity.y > 0) {
                   ...
                   // koala.position.y = tile.y - Koala.HEIGHT;
                    // we hit a block jumping upwards, let's destroy it!
                    TiledMapTileLayer layer2 = (TiledMapTileLayer)map.getLayers().get("wallsS");
                    TiledMapTileLayer layer3 = (TiledMapTileLayer)map.getLayers().get("wallsMoneda");
                    layer2.setCell((int)tile.x, (int)tile.y, null);
                    ...
                    //layer2.setCell((int)tile.x, (int)tile.y, null);
                    //layer2.setCell((int)tile.x, (int)tile.y, celdaMoneda);
					...
					//	layer3.setCell((int)tile.x, (int)tile.y, null);
						...
						if(layer3.getCell((int)tile.x, (int)tile.y) == null) {
							moneda = moneda + 1;
        ...
        //Aqui se detecta cuando el koalio cae del mapa
        if (koala.position.y <= 0)
        {
            Gdx.app.log ("muerte", "Koala Dead");
            contadorMuerte = contadorMuerte +1;
            if(contadorMuerte != 4) {
                System.out.println("aquistoy");
                koala = new Koala();
                koala.position.set(20, 20);
                Superkoalio.vidas = Superkoalio.vidas-1;
            }else{
                System.out.println("se acabo");
                Superkoalio.modo = 2;
                Superkoalio.vidas = 3;
                game.setScreen(new PrimeraPantalla(game));

            }
        }

        if(koala.position.x > 198){
